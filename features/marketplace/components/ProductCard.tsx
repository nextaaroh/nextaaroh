  ...
  return (
    <Link href={"/marketplace/" + product.slug} className="block border border-gray-200 rounded-xl overflow-hidden">
      <div className="w-full h-32 bg-gray-100 flex items-center justify-center relative">
        ...
        {isFree ? <span className="absolute top-2 left-2 bg-green-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">FREE</span> : null}
        {product.is_admin_post ? <span className="absolute top-2 right-2 bg-[#0a1a3a] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">Admin Post</span> : null}
      </div>
      <div className="p-3">
        <p className="text-sm font-medium line-clamp-2">{product.title}</p>
        <p className="text-xs text-gray-400 capitalize mt-1">{product.category.replace(/_/g, " ")}</p>
        {isFree ? <p className="text-green-600 font-bold text-sm mt-1">Free</p> : <p className="text-orange-600 font-bold text-sm mt-1">₹{priceRupees}</p>}
      </div>
    </Link>
  );
}

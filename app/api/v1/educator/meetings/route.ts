...
export async function GET() {
  ...
  const { data } = await admin.from("meetings").select("id, title, description, scheduled_at").order("scheduled_at", { ascending: false }).limit(20);
...
export async function POST(req: NextRequest) {
  ...
  const admin = createAdminClient();
  const { error } = await admin.from("meetings").insert({ title, description: description ?? null, scheduled_at, created_by: userId });
